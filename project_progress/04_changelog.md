...
2025-01-XX
- **Feature: Video Transcription**: Добавлена поддержка транскрибации видео файлов.
- **Branch**: Создана ветка `feature/video-transcription` для разработки функционала.
- **Bot Router**: Добавлена функция `_handle_video()` для обработки видео файлов. Функция извлекает аудиодорожку из видео через ffmpeg и транскрибирует её.
- **Video Support**: Добавлена обработка `message.video` и видео документов (mp4, avi, mov, mkv, webm, flv, wmv, m4v, 3gp).
